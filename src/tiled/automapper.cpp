        ...
        for (int x = min_x; x <= max_x; x++)
            if (compareLayerTo(setLayer, mLayerRuleSets,
                ...
                if (mNoOverlappingRules) {
                    ...
                    QList<QPair<TileLayer*, int> > *translationTable = mLayerList.at(r);
                    // check if there are no overlaps within this rule.
                    for (int i = 0; i < translationTable->size(); i++) {
                        QRegion ruleRegionInLayer = translationTable->at(i).first->region().intersected(rule);
                        if (appliedRegions.at(translationTable->at(i).second).intersects(ruleRegionInLayer.translated(x, y))) {
                            missmatch = true;
                            break;
                        }
                    }
                    if (!missmatch) {
                        ...
                        for (int i = 0; i < translationTable->size(); i++) {
                            QRegion ruleRegionInLayer = translationTable->at(i).first->region().intersected(rule);
                            appliedRegions[(translationTable->at(i).second)] +=
                                    ruleRegionInLayer.translated(x, y);
                        }
                    }
                } else {
                    copyMapRegion(rule, QPoint(x, y), *mLayerList.at(r));
